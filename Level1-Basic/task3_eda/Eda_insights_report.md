...
Show Image Correlation matrix heatmap - the faint Churn row confirms no single strong predictor exists.
...
Show Image Box plot confirming the 267 outlier rows found in Task 2 - visible here as individual dots beyond the whisker.
...
Show Image Near-perfect linear relationship between Total day minutes and Total day charge - visual proof of the 1.00 correlation.
...
Show Image A textbook bell-curve shape - no skew, no unusual bumps.
...
🚫 Avoid multicollinearity - when building predictive models, don't include both columns from any of the near-duplicate pairs (e.g. use only "Total day charge," not both it and "Total day minutes")
🎯 Promising features - Customer service calls, Total day minutes/charge, and Total eve minutes/charge appear to be the most promising individual features for predicting churn, based on their (comparatively) higher correlations
🤖 Model choice - since no single feature is strongly predictive alone, a model that can combine multiple weak signals (such as logistic regression or a tree-based model) is likely to perform better than relying on any one feature in isolation
🛠️ Tools Used
